...
class ClassSessions {
    ...
    public function setSessions($email, $op) {
        $this->verifyIdSessions();
        $_SESSION["login"] = true;
        $_SESSION["time"] = time();
        $_SESSION["name"] = $this->login->getDataUser($email, $op)['data']['nome'];
        $_SESSION["email"] = $this->login->getDataUser($email, $op)['data']['email'];
        $_SESSION["permition"] = $this->login->getDataUser($email, $op)['data']['permissoes'];
        if($op=="lojista"){
        $_SESSION["id_usuario"] = $this->login->getDataUser($email, $op)['data']['id_lojista'];        
        }else{
            if($op =="funcionario"){
                $_SESSION["id_usuario"] = $this->login->getDataUser($email, $op)['data']['id_funcionario'];   
            }
        }
    }
